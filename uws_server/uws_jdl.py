...
class VOTFile(JDLFile):
    ...
    def save(self, jobname):
        ...
        gen_attr = ["role", "multiplicity", "default", "content_type"]
        gen_utypes = {
    ...
    def save_old(self, jobname):
        ...
        for key in ["annotation", "doculink", "type", "subtype", "version"]:
            job_attr.append(
                '<PARAM name="{key}" datatype="char" arraysize="*" value="{value}" utype="voprov:ActivityDescription.{key}"/>'.format(
                    key=key, value=self.content.get(key, "")
                )
            )
        #     ,
        ...
        job_attr.append(
            '<PARAM name="contact_name" datatype="char" arraysize="*" value="{}" utype="voprov:Agent.name"/>'.format(
                self.content.get("contact_name", "")
            )
        )
        job_attr.append(
            '<PARAM name="contact_email" datatype="char" arraysize="*" value="{}" utype="voprov:Agent.email"/>'.format(
                self.content.get("contact_email", "")
            )
        )
        job_attr.append(
            '<PARAM name="executionduration" datatype="int" value="{}" utype="uws:Job.executionduration"/>'.format(
                self.content.get("executionduration", "1")
            )
        )
        job_attr.append(
            '<PARAM name="quote" datatype="int" value="{}" utype="uws:Job.quote"/>'.format(
                self.content.get("quote", "1")
            )
        )
                ...
                if elt.tag == f"{xmlns}GROUP":
                    ...
                    if group == "used":
                        for p in elt:
                            ...
                            if ref:
                                item = {
                                    "datatype": job_def.get("parameters")
                                    .get(ref)
                                    .get("datatype", "xs:string"),
                                    "default": job_def.get("parameters")
                                    .get(ref)
                                    .get("default"),
                                    "content_type": p.get("xtype"),
                                    "annotation": job_def.get("parameters")
                                    .get(ref)
                                    .get("annotation"),
                                    "url": "",
                                }
