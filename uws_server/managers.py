...
class Manager:
    ...
    def _make_batch(self, job, jobid_var="$$", get_input_files=None):
        ...
        for rname, r in job.jdl.content.get("generated", {}).items():
            ...
            cp_results.append(
                "\n".join(line).format(
                    rname=rname, rfname=rfname, rtype=r["content_type"]
                )
            )
            # cp_results.append(
            #     '    [ -f $wd/{fname} ]'
        ...
        batch.extend(
            [
                ...
                'echo "[`timestamp`] Copy results"',
                "copy_results",
                "if [[ -f internal_provenance.json ]]; then",
                "  cp internal_provenance.json $jd",
                "fi",
                "### CLEAN",
                "rm -rf $wd",
                "trap - SIGHUP SIGINT SIGQUIT SIGTERM ERR",
...
class SLURMManager(Manager):
    ...
    def _make_sbatch(self, job, get_input_files=None):
        ...
        duration_str = (
            str(duration).replace(" days", "").replace(" day", "").replace(", ", "-")
        )
        # Create sbatch
        sbatch = [
            "#!/bin/bash -l",
        ...
        sbatch.extend(
            self._make_batch(
                job, jobid_var="$SLURM_JOBID", get_input_files=get_input_files
            )
        )
