...
VAR_PATH = "local_var"  # store var locally for quick test, prefer e.g. '/var/opt/opus'
UWS_CLIENT_ENDPOINT = (
    "http://localhost:8080"  # For local debug server with run_client.py
)
# UWS_CLIENT_ENDPOINT = '/opus_client'  # Recommended with server (i.e. relative from BASE_URL)
UWS_SERVER_URL_JS = (
    UWS_CLIENT_ENDPOINT + "/proxy"
...
TESTUSER_DEFAULT_PW = (
    "TBD"  # to be changed after install, or defined in settings_local.py
)
...
for p in [VAR_PATH, VAR_PATH + "/logs", VAR_PATH + "/config", VAR_PATH + "/db"]:
